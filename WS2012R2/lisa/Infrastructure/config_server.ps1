...
$statefile = Get-ChildItem -Path "C:" -Recurse | Where-Object { !$PsIsContainer -and [System.IO.Path]::GetFileNameWithoutExtension($_.Name) -eq "statefile" }
if ($statefile -eq $null){
	echo $statefile > C:\Windows\Temp\statefile.txt
