...
#!/bin/bash
        ...
        if [ $memMB -ge 256 ]; then
          #for-loop starts from 0
          __iterations=`$(($memMB/256))
        else
          __iterations=0
        fi
        echo "Going to start `$__iterations instance(s) of stresstestapp each consuming 256MB memory" >> /root/RemoveUnderPressure.log 2>&1
        for ((i=0; i < `$__iterations; i++)); do
          stressapptest -M 256 -s 60 &
        done
...
{
  "Error: Memory Demand did not increase after startin stresstestapp"
  return $false
}
