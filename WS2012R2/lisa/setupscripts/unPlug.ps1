                        ...
                        [string] $vhdPath)
{
    ...
    $defaultVhdPath +=  $vmName + "-" + $sectorSize + "-" + $defaultSize + "-test.vhdx"
...
#
# Description
#   Creates a new test .vhdx hard-disk
################################################################################
function CreateVHDxDiskDrive( [string] $vmName, [string] $hvServer,
                        ...
                        [string] $defaultSize, [string] $vhdPath)
{
    ...
    elseif ($vhdxType -eq "Dynamic") {
        ...
        Write-Output "Error: Failed to create the vhdx file $($vhdxName). Unknown disk type."
        return $False
    }
    if (-not $sts)
    {
        Write-Output "Error: Failed to create the new .vhdx file: $($vhdxName)"
        return $False
    }

    $error.Clear()
    Write-Output "Info: VHDx disk drive successfully created"
    return $True
}
################################################################################
# AttachVHDxDiskDrive
                        ...
                        [string] $vhdxPath, [string] $controllerType)
{
    ...
    if ($error.Count -gt 0)
    {
        Write-Output "Error: Add-VMHardDiskDrive failed to add drive on SCSI controller"
        $error[0].Exception
        return $False
    }
    $error.Clear()
    Write-Output "Info: VHDx disk drive successfully added"
    return $True
}
...
{   
    ...
    foreach ($vhdx in $vhdxDisks)
    {
        ...
        {
            $error.Clear()
            Write-Output "Info : Removing drive $vhdxName"
            ...
            if ($error.Count -gt 0)
            {
               Write-Output "Error: Remove-VMHardDiskDrive failed to delete drive on SCSI controller "
                $error[0].Exception
                return $False
            }
            $error.Clear()
...
#
# Create 1st VHDx hard-disk
#
$path1 = NewVHDxPath $vmName $hvServer $sectorSize1 $defaultSize1
$retVal = CreateVHDxDiskDrive $vmName $hvServer $type1 $sectorSize1 $defaultSize1 $path1
if (-not $retVal)
{
    Write-Output "Error: Failed to create VHDx file with size $defaultSize1"| Tee-Object -Append -file $summaryLog

    return $False
}
Write-Output "Created VHDx with size $defaultSize1"

#
# Create 2nd VHDx hard-disk
#
$path2 = NewVHDxPath $vmName $hvServer $sectorSize2 $defaultSize2
$retVal = CreateVHDxDiskDrive $vmName $hvServer $type2 $sectorSize2 $defaultSize2 $path2
if (-not $retVal)
{
    Write-Output "Error: Failed to create VHDx file with size $defaultSize2"| Tee-Object -Append -file $summaryLog
    return $False
}
Write-Output "Created VHDx with size $defaultSize2"

#
# Attaching and removing the 1st VHDx 
#
...
$retVal = RemoveVhdx $vmName $hvServer $sectorSize1 $defaultSize1
if (-not $retVal)
{
...
$retVal = RemoveVhdx $vmName $hvServer $sectorSize2 $defaultSize2
if (-not $retVal)
{
