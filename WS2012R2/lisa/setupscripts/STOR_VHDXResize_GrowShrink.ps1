...
if (-not $vhdxDrive)
{
    "Error: VM ${vmName} does not have a $controllerType drive" | Tee-Object -Append -file $summaryLog
...
{
    "Error: $controllerType virtual disk is not a .vhdx file." | Tee-Object -Append -file $summaryLog
    "       Path = ${vhdPath}"
    return $False
}
...
if ( $controllerType -eq "IDE" -or $offline -eq "True" )
{
  ...
  {
      "Error: Test case timed out for VM to be running again!" | Tee-Object -Append -file $summaryLog
      return $False
  }
  else
  {
...
{
    "Error: VM ${vmName} sees a disk size of ${diskSize}, not the expected size of ${newVhdxGrowSize}" | Tee-Object -Append -file $summaryLog
    return $False
}
...
if (-not $?)
{
   "Error: Unable to shrink VHDX file '${vhdPath}" | Tee-Object -Append -file $summaryLog
   return $False
}
...
"Info : The guest sees the new grow size ($growDiskSize) and the new shrink size ($shrinkDiskSize)" | Tee-Object -Append -file $summaryLog
