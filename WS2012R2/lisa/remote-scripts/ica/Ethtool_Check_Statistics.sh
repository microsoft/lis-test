...
while [ $wait_for_server -gt 0 ]; do
    ...
    if [ -f ~/${server_state_file} ]; then
        ...
        if [ "$server_state" == "netperfRunning" ]; then
            break
        elif [ "$server_state" == "TestFailed" ]; then
            msg="Netperf_server.sh failed on dependency vm."
            LogMsg "$msg"
            UpdateSummary "$msg"
            UpdateTestState $ICA_TESTFAILED
            exit 1
        elif [ "$server_state" == "TestAborted" ]; then
            msg="Aborted running netperf_server.sh on dependency vm."
            LogMsg "$msg"
            UpdateSummary "$msg"
            UpdateTestState $ICA_TESTABORTED
            exit 20
...
#Get the started value of 'tx_send_full' param from statistics
...
#Get the started value of 'wake_queue'
...
if ! [ $new_send_value -gt 10 ] || ! [ $new_wake_value -eq 10 ]; then
    msg="One test or both tests failed"
    LogMsg "$msg"
    UpdateSummary "$msg"
    UpdateTestState $ICA_TESTFAILED
    exit 1
