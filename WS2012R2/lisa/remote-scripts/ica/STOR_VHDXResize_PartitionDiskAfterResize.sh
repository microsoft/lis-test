...
#
########################################################################
# STOR_VHDXResize_PartitionDiskAfterResize.sh
# Description:
...
#
(echo n; echo p; echo 2; echo ; echo ;echo w) | fdisk /dev/sdb 2> /dev/null
if [ $? -gt 0 ]; then
...
case $(LinuxRelease) in
    "SLES")
		...
		#
		mkfs -t ext3 /dev/sdb2 2> ~/summary.log
		if [ $? -gt 0 ]; then
	...
	*)
		...
		#
		mkfs -t ext4 /dev/sdb2 2> ~/summary.log
		if [ $? -gt 0 ]; then
			LogMsg "Failed to format partition"
			echo "Formating partition: Failed" >> ~/summary.log
			UpdateTestState $ICA_TESTFAILED
...
mount /dev/sdb2 /mnt 2> ~summary.log
if [ $? -gt 0 ]; then
...
(echo d; echo 2; echo w) | fdisk /dev/sdb 2> /dev/null
if [ $? -gt 0 ]; then
