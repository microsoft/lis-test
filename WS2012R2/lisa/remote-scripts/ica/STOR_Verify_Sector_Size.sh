...
# add new disk partition with "n" and showing the sector unit fortat
(echo u; echo n; echo p; echo 1; echo ; echo ; echo w) |  fdisk $driveName

startSector=`fdisk -lu $driveName | tail -1 | awk '{print $2}'`
logicalSectorSize=`fdisk -l $driveName | grep -i 'Sector size' | grep -oP '\d+' | head -1`
PhysicalSectorSize=`fdisk -l $driveName | grep -i 'Sector size' | grep -oP '\d+' | tail -1`

if [ $(($startSector%8)) -eq 0 ]; then
   echo "Check the first sector size on $driveName disk can divide 8: Success"
   UpdateSummary "Check the first sector size on $driveName disk can divide 8 : Success"
else
