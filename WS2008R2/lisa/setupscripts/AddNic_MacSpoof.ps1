...
#     This script will add a NIC to the VM and sets the mac address spoofing on for all the VMBus NICs.
#
...
foreach ($p in $params)
{
    ...
    if ($temp[0].Trim() -eq "NIC")
    {
        ...
        #
        # Set the Mac Address spoofing on for the VMBus Nic
        #
        ...
        if ($legacy -ne $true)
        {
          $NicPort = Get-VMNICport -NIC $results
