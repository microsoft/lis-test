...
function CheckRequiredParameters([System.Xml.XmlElement] $vm)
{
    ...
    if (-not $fileInfo)
    {
        "Error: The parent .vhd file ${parentVhdFilename} does not exist for ${vmName}"
        return $False
    }
...
function CreateVM([System.Xml.XmlElement] $vm, [XML] $xmlData)
{
    ...
    if ($dataValid[ $dataValid.Length - 1] -eq "True")
    {
        ...
        # Create the boot .vhd
        #
        $bootVhd = New-VHD -vhdPath $vhdFilename -parentDisk $parentVhd -server $hvServer -wait
        if (-not $bootVhd)
        {
        ...
        #
        $newDrive = Add-VMDrive $vmName -ControllerID 0 -LUN 0 -server $hvServer
        if (-not $newDrive)
        {
        ...
        #
        $sts = Add-VMDisk $vmName -ControllerID 0 -LUN 0 -vhdPath $vhdFilename -server $hvServer
        if (-not $sts)
        {
        ...
        # Add the default DVD
        #
        $newDvd = Add-VMDrive $vmName -ControllerID 1 -LUN 0 -server $hvServer -DVD
        if (-not $newDvd)
        {
            "Warn : Falied to create DVD on IDE 1, port 0"
            "       The VM will not have a DVD drive"
        }
