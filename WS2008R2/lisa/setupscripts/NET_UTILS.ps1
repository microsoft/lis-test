...
function getRandUnusedMAC([String]$hvServer,[Char]$delim)
{
	...
	# $dynMACStart = (Get-VMHost -Domain $hvServer).MacAddressMinimum
    $dynMACStart = "00155D012800"
	...
	# $dynMACEnd = (Get-VMHost -Domain $hvServer).MacAddressMaximum
    $dynMACEnd = "00155D0128FF"
	...
	$staticMacs = (get-VM -Server $hvServer | Get-VMNIC | where { $_.StaticMacAddress -like "False" }).Address
...
function getAddrCount([String]$netmask)
{
    ...
    [uint32]$count = [uint32]1 * [math]::pow([uint32]2, [uint32]$valToShift)
...
function getAddress([String]$IPv4, [String]$netmask, [int]$nth)
{
    ...
    [uint32]$nth = [uint32]$nth * [math]::pow(2,$netBits)
...
function byteSum([IpAddress]$ip)
{
    ...
    [uint32]$sum = [uint32]$ip.GetAddressBytes()[1] * [math]::pow(2, 16)
    ...
    for ($i=0; $i -lt 4; $i++)
    {
        ...
        [uint32]$sum = [uint32]$bytes[$i] * [math]::pow(2, 24 - $i*8)
...
function getNetworkBroadcast([String]$IPv4, [String]$netmask)
{
    ...
    [IpAddress]$hostmask = [uint32](binaryNOT([uint32]$subnetIP.Address))
...
function getNetworkID([String]$IP, [String]$netmask)
{
	...
	[IpAddress]$host_addr = $IP
	[IpAddress]$ipnetmask = $netmask
	
	[uint32]$netid = [uint32]$host_addr.Address -band [uint32]$ipnetmask.Address
...
function binaryNOT($intVal)
{
    $valneg = -bnot $intVal
    [string]$carnat=[Convert]::ToString($valneg, 2)
    $i=0
    $val=0
    while ($i -lt 32)
    {
        [uint32]$val += ([uint32]$carnat[$i]-48)*[math]::pow(2,31-$i)
        $i=$i+1
    }
    return $val
}