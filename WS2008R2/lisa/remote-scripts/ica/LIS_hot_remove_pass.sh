...
# LIS_hot_remove_pass.sh
#
# Script to check that pass through disk is no more reflected inside VM
...
if [ $sdCount == $diskCount ]; then
    LogMsg "constants.sh disk count ($diskCount) does match disk count from fdisk -l count ($sdCount)"
	echo "constants.sh disk count ($diskCount) does match disk count from fdisk -l count ($sdCount)" >> ~/summary.log
    UpdateTestState $ICA_TESTFAILED
    exit 30 
else
    if [ "$sdCount" == "0" ]; then
	    LogMsg "Hot Disk remove of Passthrough is succssful"
	    echo "Hot Disk remove Passthrough is succssful" >> ~/summary.log
	else
