...
# This script will verify a key is not present in the speicified pool.
...
#
# verify that the Key Value is not present in the specified pool.
#

./kvptool/kvp_client -l -p $Pool | grep "${Key}; Value : ${Value}"
if [ $? -eq 0 ]; then
LogMsg "Key value pair is present in Pool"
echo "Verify key deletion : Failed" >> ~/summary.log
UpdateTestState $ICA_TESTFAILED
exit 10
fi

echo "Verified that the ${Key} Key value is deleted from pool:${Pool}" >> ~/summary.log
