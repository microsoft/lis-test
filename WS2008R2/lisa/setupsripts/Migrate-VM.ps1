...
$error.Clear()
if ($migrationType -eq "Live")
{
	Move-ClusterVirtualMachineRole -Name $vmName -Node $destinationNode
}
else
{
...
if($stopClusterNode)
{
    ...
    $vms = Get-VM -Server $currentNode
    foreach($vm in $vms)
    {
        if($vm.__SERVER.ToLower() -eq $currentNode.ToLower())
        {
...
if ($migrationType -eq "Live")
{
	Move-ClusterVirtualMachineRole -Name $vmName -Node $currentNode
}
else
{
	Move-ClusterGroup -Name $vmName -Node $currentNode
}
