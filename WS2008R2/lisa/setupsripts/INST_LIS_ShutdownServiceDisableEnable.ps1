...
#
#####################################################################
function CheckVMState([String] $vmName, [int] $newState)
{
    $stateChanged = $False
    $vm = Get-VM -VM $vmName -Server $hvServer    
...
$vmc = Get-VMIntegrationComponent -VM $vmName -Server $hvServer
foreach($elem in $vmc)
{
    if($elem.ElementName -eq "Shutdown")
    {
        $status = $elem.EnabledState
    }
}
# $status = Get-VMIntegrationService -Server $hvServer -VM $vmName -Name Shutdown
if ($status -ne 2)
{
...
Set-VMIntegrationComponent -VM $vmName -ComponentName Shutdown -State 3 -Server $hvServer
$vmc = Get-VMIntegrationComponent -VM $vmName -Server $hvServer
foreach($elem in $vmc)
{
    if($elem.ElementName -eq "Shutdown")
...
Set-VMIntegrationComponent -VM $vmName -ComponentName Shutdown -State 2 -Server $hvServer
$vmc = Get-VMIntegrationComponent -VM $vmName -Server $hvServer
foreach($elem in $vmc)
{
    if($elem.ElementName -eq "Shutdown")
...
Start-VM -VM $vmName -Server $hvServer -Confirm:$false
if ($? -ne "True")
{
