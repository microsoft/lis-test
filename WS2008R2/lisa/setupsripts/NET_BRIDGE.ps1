...
.Synopsis
 ...
 Description:
	...
	the AddNic_MacSpoof.ps1 script directly and add it. It checks the third VM for the second NIC, and if not found, will also call the AddNic_MacSpoof.ps1 script
    ...
    2x   NIC=NIC type, Network Type, Network Name, MAC Address
            ...
            parameters as the AddNic_MacSpoof script.
    ...
    The following testParams are optional:
        ...
        LEAVE_TRAIL=yes/no
            if set to yes and the AddNic_MacSpoof.ps1 script was called from within this script for VM2, then it will not be removed
...
#
#######################################################################
echo "Main body NET_BRIDGE.ps1"
#
# Check input arguments
#
if ($vmName -eq $null)
{
    "Error: VM name is nulllll"
    return $False
}
...
foreach ($p in $params)
{
    ...
    "NIC"
    {
        ...
        #
        echo $netAdapterName[${netAdapterNameIterator}]
        echo $hvServer
        $vmSwitch = $null
        $vmSwitch = Get-VMSwitch -VirtualSwitchName $netAdapterName[${netAdapterNameIterator}] -Server $hvServer
        if (-not $vmSwitch)
        {
        ...
        #
        $vm1nic = Get-VMNIC -VM $vmName -Server $hvServer -Legacy:$false | where {$_.Address -eq $vm1MacAddress[${netAdapterNameIterator}] }
		...
		if ($legacy -ne $true)
        {
          $NicPort = Get-VMNICport -NIC $vm1nic
          $NicPort.AllowMacSpoofing = $true
          Set-WmiInstance -InputObject $NicPort  | Out-Null
        }
...
$vm3 = Get-VM -Name $vm3Name -Server $hvServer -ErrorAction SilentlyContinue
if (-not $vm3)
{
...
# hold testParam data for AddNic_MacSpoof script for vm2
$vm2testParam = $null
$vm2MacAddress = $null

# hold testParam data for AddNic_MacSpoof script for vm3
$vm3testParam = $null
$vm3MacAddress = $null
...
$nic2 = Get-VMNIC -VM $vm2Name -Server $hvServer -Legacy:$false | where { $_.SwitchName -like "$vm2switchName" }
...
else
{
	...
	if ( Test-Path ".\setupScripts\AddNic_MacSpoof.ps1")
	{
		...
		.\setupScripts\AddNic_MacSpoof.ps1 -vmName $vm2Name -hvServer $hvServer -testParams $vm2testParam
	}
	else
	{
		"Error: Could not find setupScripts\AddNic_MacSpoof.ps1 ."
		return $false
	}
...
$nic3 = Get-VMNIC -VM $vm3Name -Server $hvServer -Legacy:$false | where { $_.SwitchName -like "$vm3switchName" }
...
if ($nic3) 
{
	...
	$vm3MacAddress = $vm3nic | select -ExpandProperty Address
...
else
{
    ...
    if (-not $retVal)
    {
        ...
        if ($scriptAddedNIC)
        {
            if ([string]::Compare($leaveTrail, "yes", $true) -ne 0)
            {
                if (Test-Path ".\setupScripts\RemoveNic.ps1")
                {
                    .\setupScripts\RemoveNic.ps1 -vmName $vm2Name -hvServer $hvServer -testParams $vm2testParam
                }
                else
                {
                    "Warning: Unable to find setupScripts\RemoveNic.ps1 in order to remove the added NIC"
                }
            }
        }
        return $false
	...
	#construct AddNic_MacSpoof Parameter
	...
	if ( Test-Path ".\setupScripts\AddNic_MacSpoof.ps1")
	{
		...
		.\setupScripts\AddNic_MacSpoof.ps1 -vmName $vm3Name -hvServer $hvServer -testParams $vm3testParam
	}
	else
	{
		"Error: Could not find setupScripts\AddNic_MacSpoof.ps1 ."
	...
	# get the newly added NIC
	$vm3nic = Get-VMNIC -VM $vm3Name -Server $hvServer -Legacy:$false | where { $_.Address -like "$vm3MacAddress" }
    ...
    if (-not $retVal)
    {
        ...
        if ($scriptAddedNIC)
        {
            if ([string]::Compare($leaveTrail, "yes", $true) -ne 0)
            {
                ...
                {
                    .\setupScripts\RemoveNic.ps1 -vmName $vm2Name -hvServer $hvServer -testParams $vm2testParam
                    .\setupScripts\RemoveNic.ps1 -vmName $vm3Name -hvServer $hvServer -testParams $vm3testParam
                }
                else
                {
                    "Warning: Unable to find setupScripts\RemoveNic.ps1 in order to remove the added NIC"
...
{
	Start-VM -VM $vm3Name -Server $hvServer
	if (-not $?)
	{
